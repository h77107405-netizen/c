...
export const TeacherTestsPage: React.FC = () => {
  ...
  const [form, setForm] = useState({ title: '', batchId: '', totalMarks: '100', duration: '60', scheduledAt: '', instructions: '' });

  const load = () => {
    setLoading(true);
    Promise.all([api.teacher.getTests(), api.teacher.getBatches()]).then(([t, b]) => {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.teacher.createTest(form);
      toast.success('Test created');
      setAddOpen(false);
      setForm({ title: '', batchId: '', totalMarks: '100', duration: '60', scheduledAt: '', instructions: '' });
      load();
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            <DialogTrigger asChild>
              <Button className="bg-gradient-to-r from-indigo-600 to-purple-600"><Plus className="h-4 w-4 mr-2" /> Create Test</Button>
            </DialogTrigger>
            <DialogContent>
              ...
              <form onSubmit={handleAdd} className="space-y-4 mt-4">
                ...
                <div><Label>Schedule</Label><Input type="datetime-local" value={form.scheduledAt} onChange={(e) => setForm({...form, scheduledAt: e.target.value})} /></div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableRow><TableHead>Title</TableHead><TableHead>Batch</TableHead><TableHead>Total Marks</TableHead><TableHead>Duration</TableHead><TableHead>Scheduled</TableHead><TableHead>Status</TableHead><TableHead>Action</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {tests.map((t) => (
                  <TableRow key={t.id}>
                    ...
                    <TableCell>
                      {t.status === 'draft' && (
                        <Button size="sm" onClick={() => handlePublish(t.id)}>Publish</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
                {tests.length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No tests created yet</TableCell></TableRow>}
              </TableBody>
            </Table>
          </CardContent>
        </Card>
